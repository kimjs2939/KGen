...
class TypeDeclarationStatement(Statement):
    ...
    def process_item(self):
        ...
        assert line.lower().startswith(clsname),'{},{}'.format(line,clsname)
    ...
    def _parse_entity(self, line):
        m = name_re(line)
        assert m,'{},{},{}'.format(line,self.item,self.__class__.__name__)
        name = line[:m.end()]
...
class Implicit(Statement):
        ...
        for item in split_comma(line, self.item):
            ...
            for spec in split_comma(item[i+1:-1].strip(), self.item):
                if '-' in spec:
                    ...
                    s = s.strip()
                    e = e.strip()
                    assert s in self.letters and e in self.letters,'{},{}'.format(s,e)
                else:
                    e = s = spec.lower().strip()
                    assert s in self.letters,'{},{}'.format(s,e)
                specs.append((s,e))
            tspec = item[:i].rstrip()
            stmt = None
            for cls in declaration_type_spec:
                if cls.match(tspec):
                    ...
                    if stmt.isvalid:
                        break
            assert stmt is not None,'{},{}'.format(item,line)
    ...
    def analyze(self):
        ...
        for stmt,specs in self.items:
            for s,e in specs:
                for l in string.ascii_lowercase[string.ascii_lowercase.index(s.lower()):\
                                          string.ascii_lowercase.index(e.lower())+1]:
